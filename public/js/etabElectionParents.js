...
function afficheAcademieDepartementCommune(nomForm) {
    ...
    $.post(url_getAcaDepComParZone, {
        ...
        if (data.responseCode == 200) {
            ...
            if (data.liste_departement.length > 1) {

                
                var indice = 0;
                select_departement.options[indice] = new Option('Tous', '');
                indice++;
                ...
                select_departement.options.length = indice;
            } else if (data.liste_departement.length == 1) {
                // CAS CE/DE/DSDEN : 1 seul département
                select_departement.options[0] = new Option(data.liste_departement[0]['libelle'], data.liste_departement[0]['numero']);
                select_departement.options[0].setAttribute("selected", "selected");
            }

            if (data.liste_commune.length > 1) {
                var commune_selectionne = document.getElementById("commune_selectionne").value;

                var indice = 0;
                select_commune.disabled = false;
                select_commune.options[indice] = new Option('Toutes', '');
                indice++;

                for (var i in data.liste_commune) {
                    select_commune.options[indice] = new Option(data.liste_commune[i]['libelle'] + ' (' + data.liste_commune[i]['cp'] + ')', data.liste_commune[i]['id']);
                    if (data.liste_commune[i]['id'] == commune_selectionne) {
                        select_commune.options[indice].setAttribute("selected", "selected");
                    }
                    indice++;
            ...
            } else if (data.liste_commune.length == 1) {
                // CAS CE/DE : 1 seule commune
                select_commune.options[0] = new Option(data.liste_commune[0]['libelle'] + ' (' + data.liste_commune[0]['cp'] + ')', data.liste_commune[0]['id']);
                select_commune.options[0].setAttribute("selected", "selected");
            }

            if (data.liste_etablissement.length > 1) {
                var indice = 0;
                select_etablissement.disabled = false;
                select_etablissement.options[indice] = new Option('Tous', '');
                indice++;
                for (var i in data.liste_etablissement) {
                    select_etablissement.options[indice] = new Option('(' + data.liste_etablissement[i]['uai'] + ')' + ' - ' + data.liste_etablissement[i]['libelle'] + ' - ' + data.liste_etablissement[i]['commune'], data.liste_etablissement[i]['uai']);
                    if (data.liste_etablissement[i]['uai'] == etablissement_selectionne.val()) {
                        select_etablissement.options[indice].setAttribute("selected", "selected");
                    }
                    indice++;
                }
            ...
            } else if (data.liste_etablissement.length == 1) {
                // CAS CE/DE : 1 seul établissement (pour le moment)
                select_etablissement.options[0] = new Option('(' + data.liste_etablissement[0]['uai'] + ')' + ' - ' + data.liste_etablissement[0]['libelle'] + ' - ' + data.liste_etablissement[0]['commune'], data.liste_etablissement[0]['uai']);
                select_etablissement.options[0].setAttribute("selected", "selected");
                etablissement_selectionne.val(data.liste_etablissement[0]['uai']);
            }

            if (departement_selectionne == "") {
                select_commune.options.length = 1;
                select_commune.options[0] = new Option('Toutes', '');
                select_commune.disabled = true;

                select_etablissement.options.length = 1;
                select_etablissement.options[0] = new Option('Aucun', '');
                select_etablissement.disabled = true;
            }
