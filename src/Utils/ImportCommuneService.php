...
class ImportCommuneService
{
    ...
    {    	
    	try {
            ...
            $logger->info("Debut import fichier des communes...");
            $i = 0;
            if (strpos(basename($url), $refComPattern) === 0) {
                ...
                $nbLigneNonTraite = 0;
                $nbDoublonsFichier = 0;
                $arrayRefCommune = $this->em->getRepository('EPLEElectionBundle:RefCommune')->getArrayRefCommuneByCodeInsee();
                $fh = fopen($url, 'r');
                $dataFile = array();
                $result = array();
                ...
                while ($ligne = utf8_encode(fgets($fh))) {
                    ...
                    if (
                    	...
                    	if ($this->traitementCodeDept($datas[4]) == null){
                    		$nbLigneNonTraite++;
                    		$logger->info("erreur de département: [".$datas[0]."] [".$datas[1]."] [".$datas[2]."] [".$datas[4]."]");
                    	} else {
                    	...
                    	if (is_array($datas) && sizeof($datas) == 5){
                    		...
                    		$logger->info("Cette commune n’a pas pu être traitée correctement : [".$logCodeInsee."] [".$logLibelle."] [".$logCodePostal."] [".$logDepartement."]");
                    		//$logger->info("le traitement ne s’arrête pas à la première erreur : tous les enregistrements du fichier des communes sont traités ");
                    		$nbErrorFormatLigne++;
                    	}
                    }
                ...
                // $logger->info("Nombre de code_insee en doublons dans le fichier CSV: " . $nbDoublonsFichier);
                ...
                $countRefCommuneInDbAfterImport = $this->em->getRepository('EPLEElectionBundle:RefCommune')->countAllCommunes();

                if( is_array($result['arrayDeptErrorAjout']) && !empty($result['arrayDeptErrorAjout'])){
                	foreach($result['arrayDeptErrorAjout'] as $value){
                		$logger->info("erreur de départements causant un echec d'insertion de communes: [".$value['code_insee']."] [".$value['libelle']."] [".$value['code_postal']."]" );
                	}
                }
                if( is_array($result['arrayDeptErrorModification']) && !empty($result['arrayDeptErrorModification'])){
                	foreach($result['arrayDeptErrorModification'] as $value){
                		$logger->info("erreur de départements causant un echec de modification de communes: [".$value['code_insee']."] [".$value['libelle']."] [".$value['code_postal']."]" );
                	}
                }
                // 0240032: Batch RAMESE - Erreur dans la log - Problème de compteurs
                $logger->info($nbCommuneInFile. " communes lues dans le fichier CSV");
                $logger->info($nbDoublonsFichier. " communes en doublon sur le code INSEE dans le fichier CSV");
                $logger->info($nbErrorFormatLigne. " communes avec un mauvais format");
                $logger->info($nbLigneNonTraite. " communes non traitées");
                $logger->info($result['nb_communes_mise_a_jour']. " communes mises à jour dans la table ref_commune");
                if($result['nb_communes_doublons'] > 0) {
                    $logger->info($result['nb_communes_doublons']. " communes en doublons mises à jour dans la table ref_commune");
                }
                $logger->info($result['nb_communes_ajoutees'] . " communes insérées dans la table ref_commune");
                $logger->info($result['nb_communes_inchangees'] . " communes lues et inchangées dans la table ref_commune ");
                if ($result['nb_error_departement_modification'] > 0) {
                    $logger->info($result['nb_error_departement_modification'] . " erreurs de modification de communes causée(s) par des erreurs département ");
                }
                if ($result['nb_error_departement_ajout'] > 0) {
                    $logger->info($result['nb_error_departement_ajout'] . " erreurs d'ajout de communes causée(s) par des erreurs département ");
                }
                $logger->info($countRefCommuneInDbAfterImport. " communes présentes en table ref_commune en fin de traitement ");
            ...
            $logger->info("Fin import fichier communes");
        } catch (Exception $e) {
            $logger->error('KO :' . $e->getMessage());
        }
    }
    ...
    private function insertOrMajCommunes($datas){
        ...
        if(is_array($datas['new']) && !empty($datas['new'])){
            foreach($datas['new'] as $key => $value){
                $departementRepo = $this->em->getRepository('EPLEElectionBundle:RefDepartement')->find($datas['new'][$key]["departement"]);
        ...
        if(is_array($datas['maj']) && !empty($datas['maj'])){
            foreach($datas['maj'] as $key => $value){
                $departementRepo = $this->em->getRepository('EPLEElectionBundle:RefDepartement')->find($datas['maj'][$key]["departement"]);
                if ($departementRepo instanceof RefDepartement && !empty($departementRepo) && strlen($departementRepo->getNumero()) <= 3) {
                    $isExist = $this->em->getRepository('EPLEElectionBundle:RefCommune')->findBy(array('codeInsee' => $key));
        ...
        if (sizeof($arrayCommuneToUpdate) > 0) {
            $this->em->getRepository('EPLEElectionBundle:RefCommune')->insertListeRefCommunesByImport($arrayCommuneToUpdate);
        }

        if (sizeof($arrayCommuneToInsert) > 0) {
            $this->em->getRepository('EPLEElectionBundle:RefCommune')->insertListeRefCommunesByImport($arrayCommuneToInsert);
        }
        $result = array(
