...
abstract class EpleUtils {
	 ...
	 * @return TOUTES_ZONES ou null ou App\Entity\RefAcademie ou App\Entity\RefDepartement
	 */
	public static function getZone($em, $idZone = null) {
		if ($idZone === self::TOUTES_ZONES) {
			$zone = self::TOUTES_ZONES;
		} else {
			$zone = $em->getRepository('EPLEElectionBundle:RefDepartement')->find($idZone);
			if ($zone == null) {
				$zone = $em->getRepository('EPLEElectionBundle:RefAcademie')->find($idZone);
			}
			if ($zone == null) {
			    $zone = $em->getRepository('EPLEElectionBundle:RefEtablissement')->find($idZone);
			}
		}
		return $zone;
	}
