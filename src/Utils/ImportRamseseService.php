...
class ImportRamseseService
{
    ...
    private $logger; // Log
    public function __construct(EntityManagerInterface $em, ContainerInterface $container,LoggerInterface $importLogger)
    {
        $this->em = $em;
    ...
    public function import($url)
    {
        ...
        $this->logger->info("nb total distinct uai eleEtablissement : " . sizeof($arrayEleEtablissement));
        ...
        $this->logger->info("nb refZoneNature :" . sizeof($arrayRefZoneNature));
        ...
        $this->logger->info("nb refTypeEtablissement :" . sizeof($arrayRefTypeEtablissement));

        $arrayRefCommune = $this->em->getRepository(RefCommune::class)->getArrayRefCommune();
        $this->logger->info("nb refCommune :" . sizeof($arrayRefCommune));
        ...
        if (strpos(basename($url), $etabPattern) === 0) {
                ...
                if (sizeof($datas) == 37) { // on verifie si le fichier contient bien 36 infos
                    if (strlen($datas[0]) == 8) { // on verifie si la premiere info du fichier est bien de 8 caractere (car longth(UAI) = 8)
                        ...
                        if ($datas[6] == 'PU') {
                                ...
                                if ($datas[1] == "I") {
                                    ...
                                    if (array_key_exists($datas[3], $arrayRefZoneNature)) {
                                        ...
                                        if ($zoneNature->getTypeNature() == $datas[4]) {
                                            ...
                                            if ($refId == 0) {
                                                ...
                                                if (!empty($datas[21])) {
                                                    ...
                                                    if (!empty($datas[19]) AND $datas[19] != 1) {
                                                        ...
                                                        $this->logger->info('Valeur du code : '.$datas[19]);
                                                        $this->logger->info('Date limite : '.$dateLimiteFermeture2->format('Y-m-d'));
                                                        $this->logger->info('Date fermeture : '.$dateFermeture->format('Y-m-d'));

                                                        if ($dateFermeture > $dateLimiteFermeture2) {
                                                            $this->logger->info('Cas actif '.$datas[0]);
                                                            $refEtab->setDateFermeture($dateFermeture);
                                                            $refEtab->setActif(1);
                                                        } else {
                                                            $this->logger->info('Cas inactif '.$datas[0]);
                                                            $refEtab->setDateFermeture($dateFermeture);
                                                            $refEtab->setActif(0);
                                                        }
                                                    }
                                            ...
                                            $this->logger->error($datas[0] . ": impossible de traiter ce type d'établissement, le code nature et le type nature ne sont pas cohérent.");
                                            $retour['nbEtabRejete'] ++;
                                        }
                                    } else {
                                        $this->logger->error($datas[0] . ": impossible de traiter ce type d'établissement, le code nature est inconnu.");
                                        $retour['nbEtabRejete'] ++;
                                    }
                                } else {
                                    $this->logger->error($datas[0] . ": impossible de traiter ce type d'établissement");
                                    $retour['nbEtabRejete'] ++;
                                }
                            ...
                            $this->logger->error($datas[0] . ": Cet établissement est un établissement non public.");
                            $retour['nbEtabRejete'] ++;
                        }
                    } else {
                        $this->logger->error($datas[0] . ": l'UAI de cet établissement est incorrecte, vérifier le nombre de caractére du champ UAI");
                        $retour['nbEtabRejete'] ++;
                    }
                } else {
                    $this->logger->error($datas[0] . ": la ligne des informations pour cet établissement est incorrecte, vérifier le nombre de champs de données");
                    $retour['nbEtabRejete'] ++;
                }
            }
            fclose($fh);
            ...
            $this->logger->info("Résultat du traitement : " . $retour['nbEtabUpdate'] . " établissements sont mis à jour, " . $retour['nbEtabAdd'] . " ajoutés et " . $retour['nbEtabRejete'] . " rejetés.");
            ...
            if (strpos(basename($url), $commPattern) === 0) {
                ...
                while ($ligne = utf8_encode(fgets($fh))) {
                    ...
                    if ($datas[0] == 'L' && $datas[3] == 10) {
                        ...
                        if (null == $uai) {
                            $this->logger->error($id_ligne . " : UAIRNE est vide");
                        } elseif (null == $ident) {
                            $this->logger->error($id_ligne . " : IDENT est vide");
                        } else {
                            $array = array();
                            $array['uai'] = $uai;
                            $array['insee'] = $ident;
                        ...
                        if (array_key_exists($datas[2], $arrayRefEtablissement)) { // on verifie si UAI de l'etablissement existe dans la BD
                            if (!empty($datas[3]) && ($datas[3] == '35' || $datas[3] == '36')) { // on traite que les types prio 35 et 36
                                ...
                                $this->logger->error($id_ligne . " : le code ne correspont pas ni à REP ni à REPPLUS.");
                            }
                        } else {
                            $this->logger->error($id_ligne . " : l'UAI n'existe pas en base.");
                        }
                    }
                }
                fclose($fh);
                ...
                if (sizeof($listeUaisInsee) > 0) {
                    ...
                    foreach ($listeUaisInsee as $key => $array) {
                        ...
                        if (array_key_exists($array['insee'], $idCommunes)) {
                            ...
                            $this->logger->error("La commune pour le code insee " . $array['insee'] . " n'a pas été trouvée");
                ...
                $this->logger->info("Avant purge : " . sizeof($listeUaisInseeCommunes) . " lignes à traiter");
                ...
                $this->logger->info("Après purge : " . sizeof($listeDonneesAMettreAJour) . " lignes à traiter");
                ...
                // resultat du traitement
                $this->logger->info("Résultat du traitement : " . $retour['nbEtabUpdate'] . " établissements traité(s) et " . $retour['nbCommunesNonTrouvees'] . " établissements n'ont pas été mis à jour suite à un code insee introuvable.");
            } else {
        ...
        $this->logger->info("Fin import fichier");
        return $retour;
    }
