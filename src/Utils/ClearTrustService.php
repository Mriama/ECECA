...
class ClearTrustService{
	...
	// Type d'élection
// 	const ECECA_PE = 'ECECA-PE';
// 	const ECECA_RP = 'ECECA-RP'; 
    ...
    public function login($request){
    	...
    	$wsdl_location = $this->params->get("wsdl_location");
    	$wsdl_uri = $this->params->get("wsdl_uri");
    	...
    	try
    	{
    		...
    		if(0 != strcmp(self::FONCT_ADM_DIR, $FrEduFonctAdm) && null != $FrEduResDel && preg_match("`ECECA(.*)redirectionhub`", $FrEduResDel) && $this->checkDateFrEduResDel($FrEduResDel))
    		{
	    		...
	    		/* else{
	    			...
	    			$message = "FrEduResDel ne contient pas ECECA|/redirectionhub";
	    			$this->logger->error($message);
	    			throw new \Exception($message);
	    		}*/ 
    	...
    	// TODO Liste des utilisateurs à garder en base
    ...
    /**
     * TODO YME - Persister les n°s d'académies.
     * @param unknown $login
     * @return string
     */
    ...
    private function getUaisFromFrEduResDel($FrEduResDel){
    	...
    	foreach ($FrEduResDel as $FrEduRneResp){
    		$uai = substr($FrEduRneResp,0,self::UAI_LENGTH);
    		if(preg_match("@([0-9]{7}[A-Z])@", $uai)){
    			array_push($result, $uai);
    		}
    	}
    	return $result;
    }*/
    ...
    private function getUaisFromFrEduRneResp($FrEduRneResp){
    	...
    	foreach($uaiArray as $uaiTmp){
    		array_push($result, substr($uaiTmp,0,self::UAI_LENGTH));
    	}
    	return $result;
    }*/
    ...
    private function split($haystack, $needle){
    	...
    	$sentences = array();
    	$parts = preg_split('/'.$needle.'/', $haystack, -1, PREG_SPLIT_DELIM_CAPTURE);
    	for ($i=0, $n=count($parts)-1; $i<$n; $i+=2) {
    		$sentences[] = $needle.$parts[$i].$parts[$i+1];
    	}
    	if ($parts[$n] != '') {
    		$sentences[] = $needle.$parts[$n];
    	}
