...
class SaisieResultatController extends AbstractController {
    ...
    public function editionAction($etablissementUai, $codeUrlTypeElect, $retourLstRech)
    {
        ...
        if ($this->request->getMethod() == 'POST') {
            ...
            if ($form->isSubmitted() && $form->isValid()) {
                ...
                if (($nbOrganisation - $nbVoixNul) != 0) {
                    ...
                    $listeResultatsDetailles = $em->getRepository(EleResultatDetail::class)->findBy(array('electionEtab'=>$datasEleEtablissement->getId()));
                    ...
                    if ($datasParticipation->getNbSiegesPourvus() < $datasParticipation->getNbSiegesPourvoir()) {
                        ...
                        $typeAlerte = $em->getRepository(RefTypeAlerte::class)->find(RefTypeAlerte::CODE_DEFICIT);
                    ...
                    if ($datasParticipation->getNbInscrits() >= 0 && $nbTotalCandidats == 0
                        ...
                        $typeAlerte = $em->getRepository(RefTypeAlerte::class)->find(RefTypeAlerte::CODE_CARENCE);
                        $eleAlerte = new EleAlerte();
                        $eleAlerte->setElectionEtab($datasEleEtablissement);
                        $eleAlerte->setTypeAlerte($typeAlerte);
                        $em->persist($eleAlerte);
        ...
        $params['confirmDeficit'] = $this->getParameter('message_deficit_confirm');
        // 014E externalisation du message bloquant RG_SAISIE_135
        $params['msgCandidatTitulaire'] = $this->getParameter('message_candidat_titulaire');


        return $this->render('saisieResultat/saisieResultats.html.twig', $params);
    }
     ...
     */
    public function aideNbSiegesAction($etablissementUai, $typeElectionId)
    {
        $em = $this->doctrine->getManager();
        ...
        if($etablissement->getCommune()->getDepartement()->getAcademie()->getCode() == RefAcademie::CODE_ACA_MAYOTTE){
            ...
            $settings = $this->getParameter('nb_sieges_typeElection_typeEtab_mayotte');
        ...
        $params['libelle_aide_ecole'] = $this->getParameter('libelle_aide_ecole');
        $params['libelle_aide_college_moins_600'] = $this->getParameter('libelle_aide_college_moins_600');
        $params['libelle_aide_college'] = $this->getParameter('libelle_aide_college');
        $params['libelle_aide_lycee'] = $this->getParameter('libelle_aide_lycee');
        $params['libelle_aide_erea'] = $this->getParameter('libelle_aide_erea');
        $params['libelle_aide_erpd'] = $this->getParameter('libelle_aide_erpd');
        $params['libelle_aide_erea_et_erpd'] = $this->getParameter('libelle_aide_erea_et_erpd');
        $params['libelle_aide_indiquer_nombre_classes'] = $this->getParameter('libelle_aide_indiquer_nombre_classes');
        $params['libelle_aide_nombre_classes'] = $this->getParameter('libelle_aide_nombre_classes');
        $params['libelle_aide_question_college'] = $this->getParameter('libelle_aide_question_college');
        $params['libelle_aide_indiquer_type_etablissement'] = $this->getParameter('libelle_aide_indiquer_type_etablissement');
        $params['nombre_limite_ecole'] = $this->getParameter('nombre_limite_ecole');
        $params['message_erreur_nombre_limite_ecole'] = $this->getParameter('message_erreur_nombre_limite_ecole');
        $params['libelle_aide_titre'] = $this->getParameter('libelle_aide_titre');


        return $this->render('saisieResultat/aideNbSiegesAPourvoir.html.twig', $params);
    }
     ...
     */
    public function nbSiegesTirageAuSortAction($uai, $codeUrlTypeElect)
    {
        $em = $this->doctrine->getManager();
        ...
        $params['warning'] = $this->getParameter('warning');
        return $this->render('saisieResultat/nbSiegesTirageAuSort.html.twig', $params);
    }
