...
class EleConsolidationRepository extends EntityRepository {
    ...
    public function queryFindDatasConsolidationOfResultatsGlobauxByCampagneZoneTypeEtab(EleCampagne $campagne, $zone = EpleUtils::TOUTES_ZONES, $typeEtab = "tous", $perimetre = null) {
        ...
        $stringQuery = "	SELECT	camp.id, sum(p.nbInscrits) as nbIns, sum(p.nbVotants) as nbVotants, sum((p.nbVotants-p.nbNulsBlancs)) as nbExpr,
						...
						FROM EPLEElectionBundle:EleConsolidation cons
    ...
    public function getEleConsolidationGlobale($campagne, $typeEtab = null, $zone = null, $refUser = null) {
        ...
        // Résultats globaux
        $resultatsGlobaux = $this->_em->getRepository('EPLEElectionBundle:EleResultat')
        ...
        $resultatsDetailGlobaux = $this->_em->getRepository('EPLEElectionBundle:EleResultatDetail')
        ...
        if (!empty($resultatsGlobaux)) {
            ...
            $fct_datasToEleResult = function ($datas) use ($em) {
                $r = new EleResultat();
                $r->setOrganisation($em->getRepository('EPLEElectionBundle:RefOrganisation')->find($datas['idOrg']));
    ...
    public function queryFindDatasConsolidationOfResultatsGlobauxByCampagneNiveauTypeEtab($campagne, $zone, $typeEtab = "tous") {
        ...
        $academie = $this->_em->getRepository('EPLEElectionBundle:RefAcademie')->find($zone);
...
//         GROUP BY ec.id_campagne, rd.libelle
        ...
        $stringQuery = '
        		...
        		JOIN EPLEElectionBundle:EleCampagne eleCamp WITH eleCamp.id = eleConso.campagne
                JOIN EPLEElectionBundle:EleParticipation elePart WITH elePart.id = eleConso.participation
        		JOIN EPLEElectionBundle:RefTypeEtablissement refTypeEtab WITH refTypeEtab.id = eleConso.typeEtablissement    
                JOIN EPLEElectionBundle:RefDepartement refDept WITH eleConso.idZone = refDept.numero
