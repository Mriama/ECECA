...
class EtablissementHandler {
		...
		if ($this->request->getMethod() == 'POST') {
			...
			if ($this->form->isSubmitted() && $this->form->isValid()) {
				...
				if ($mon_etablissement->getFlagAddCommune() === 'true') {
					...
					$commune_existante = $this->em->getRepository(RefCommune::class)->findOneBy(array('libelle' => $libelle_commune, 'codePostal' => $cp_commune));
	...
	public function onSuccess(RefEtablissement $etablissementForm) {
		...
		$nbCommuneDelete = $this->em->getRepository(RefCommune::class)->deleteCommuneSansEtab();
	}
}