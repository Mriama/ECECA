...
class ImportRamseseService
{
    ...
    public function import($url)
    {
       ...
       // Recuperation de UAI de tous les etablissements
        $arrayRefEtablissement = $this->em->getRepository('EPLEElectionBundle:RefEtablissement')->getArrayRefEtablissementUai();
        $logger->info("nb uai refEtablissement : " . sizeof($arrayRefEtablissement));
    	
        
      //  Recuperation de ID commune de tous les etablissements
        $arrayRefEtablissementCommune = $this->em->getRepository('EPLEElectionBundle:RefEtablissement')->getArrayRefEtablissementUaiIdCommune();
        
        //  Recuperation de ID type prioritaire de tous les etablissements
        $arrayRefEtablissementTypePrioritaire = $this->em->getRepository('EPLEElectionBundle:RefEtablissement')->getArrayRefEtablissementUaiIdTypePrioritaire();
        
        //Recuperation de ID commune de tous les etablissements
        $arrayEleEtablissement = $this->em->getRepository('EPLEElectionBundle:EleEtablissement')->getArrayEleEtablissementUai();
        $logger->info("nb total distinct uai eleEtablissement : " . sizeof($arrayEleEtablissement));
        
        $listeRefZoneNature = $this->em->getRepository('EPLEElectionBundle:RefZoneNature')->findAll();
        ...
        $listeRefTypeEtablissement = $this->em->getRepository('EPLEElectionBundle:RefTypeEtablissement')->findAll();
        ...
        $arrayRefCommune = $this->em->getRepository('EPLEElectionBundle:RefCommune')->getArrayRefCommune();
        ...
        if (strpos(basename($url), $etabPattern) === 0) {
	                            		...
	                            		if (array_key_exists($datas[3], $arrayRefZoneNature)) {
	                            			...
	                            			if ($zoneNature->getTypeNature() == $datas[4]) {
	                            				...
	                            				if ($refId == 0) {
	                            					...
	                            					$lastTypeEtablissement = $this->em->getRepository('EPLEElectionBundle:RefEtablissement')->find($refEtabUai)->getTypeEtablissement();
            ...
            if (sizeof($listeRefEtablissementToInsert) > 0) {
                $this->em->getRepository('EPLEElectionBundle:RefEtablissement')->insertListeRefEtablissementByRamsese($listeRefEtablissementToInsert);
            }
            
            if (sizeof($listeRefEtablissementToUpdate) > 0) {
            	$this->em->getRepository('EPLEElectionBundle:RefEtablissement')->insertListeRefEtablissementByRamsese($listeRefEtablissementToUpdate);
            }
            ...
            /*if (sizeof($listeRefEtablissementToRemove) > 0) {
                $this->em->getRepository('EPLEElectionBundle:RefEtablissement')->removeListeRefEtablissementByRamsese($listeRefEtablissementToRemove);
            }*/
            
            /*if (sizeof($listeRefEtablissementToDesactive) > 0) {
                $this->em->getRepository('EPLEElectionBundle:RefEtablissement')->desactiveListeRefEtablissementByRamsese($listeRefEtablissementToDesactive);
            }*/
            ...
            if (strpos(basename($url), $commPattern) === 0) {
                ...
                if (sizeof($listeUaisInsee) > 0) {
                	$idCommunes = $this->em->getRepository('EPLEElectionBundle:RefCommune')->findIdParCodeInsee($listeUaisInsee);
                ...
                if (sizeof($listeDonneesAMettreAJour) > 0) {
                    $this->em->getRepository('EPLEElectionBundle:RefEtablissement')->updateListeRefEtablissementCommuneByRamsese($listeDonneesAMettreAJour);
                }

                // Mise à jour des types prioritaire dans la base
                if (sizeof($listeUaisPrioritaires) > 0) {
                	$this->em->getRepository('EPLEElectionBundle:RefEtablissement')->updateListeRefEtablissementTypePrioritaireByRamsese($listeUaisPrioritaires);
                }
